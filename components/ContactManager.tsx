...
import { 
  ...
  Tag as TagIcon, Hash, Link as LinkIcon, Database, ArrowUpRight, FileSpreadsheet
} from 'lucide-react';
...
const ContactManager: React.FC<ContactManagerProps> = ({ category }) => {
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-700 h-full flex flex-col">
      <div className="bg-white/80 backdrop-blur-2xl p-10 rounded-[48px] border border-white shadow-[0_20px_50px_rgba(0,0,0,0.04)] space-y-8 shrink-0">
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-8">
           <div className="flex items-center gap-8">
              <div className={`w-20 h-20 rounded-[30px] bg-gradient-to-tr ${gradientClass} flex items-center justify-center text-white shadow-2xl rotate-3`}>
                 <Users size={32} strokeWidth={2.5} />
              </div>
              <div>
                 <h2 className="text-3xl font-black uppercase italic tracking-tighter text-slate-900 leading-tight">Master Database <span className={`text-${themeColor}-600`}>{category}s</span></h2>
                 <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.4em] mt-2 italic flex items-center gap-2">
                   <div className={`w-2 h-2 rounded-full bg-${themeColor}-500 animate-pulse`}></div> Cloud Node v13.1 (UUID Fixed)
                 </p>
              </div>
           </div>
           
           <div className="flex items-center gap-3">
              <button onClick={fetchContacts} className="p-5 bg-white border border-slate-100 text-slate-400 hover:text-indigo-600 rounded-2xl transition-all shadow-sm active:scale-90">
                 <RefreshCw size={22} className={loading ? "animate-spin" : ""} />
              </button>
              <button 
                onClick={() => { setEditingContact({ id: 'new' } as any); setIsModalOpen(true); }}
                className={`px-10 py-5 bg-gradient-to-r ${gradientClass} text-white rounded-[28px] font-black text-[11px] uppercase tracking-[0.2em] shadow-xl flex items-center gap-3 transition-all hover:scale-[1.03] active:scale-[0.97] italic`}
              >
                <Plus size={20} strokeWidth={4}/> Ajouter un {category}
              </button>
           </div>
        </div>
        ...
        <div className="flex flex-col md:flex-row gap-4 items-center">
           <div className="relative flex-1 w-full group">
              ...
              <input 
                ...
                className="w-full pl-16 pr-6 py-5 bg-slate-50/50 border border-slate-100 rounded-[28px] text-[13px] font-black uppercase tracking-tight italic outline-none focus:ring-8 focus:ring-indigo-500/5 focus:bg-white transition-all shadow-inner"
                value={searchTerm}
              ...
              <div key={c.id} className="bg-white/80 backdrop-blur-md rounded-[40px] border border-slate-100 shadow-sm hover:shadow-xl hover:-translate-y-1.5 transition-all duration-500 flex flex-col overflow-hidden relative group">
                ...
                <div className="p-8 flex-1">
                  <div className="flex justify-between items-start mb-6">
                    ...
                    <span className={`px-4 py-1.5 rounded-full text-[8px] font-black uppercase tracking-[0.2em] border italic ${
                      ['Active', 'Closed', 'Interested'].includes(c.status) 
